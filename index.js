...
let wordList2 = ["bayot", "wasak", "salsa", "fruit", "gulay", "pigsa", "yagba", "biyak", "bebeh", "buang", "gwapo", "ganda", "seksi", "sixty", "bakla", "Gspot", "tarub", "dildo"]
let row
let tiles = document.getElementsByClassName('col')
let keyboardKeys = document.getElementsByClassName("keys")
let randomIndex = [0,1,2,3,4,5]
let col
let keys, GameRunning
let word
let tileComplete = false
let guess = ""
let c, r, e, i
let maximum, permaximum, renewC, renewR
let consIndex = 5;
let newIndexes = []
let gold 
let currentRow
let hint_box = document.getElementById("hint-box")
...
function hintOption(par){
    if(par == 0){
        hint_box.style.display = "none"
    } else if(par == 1 && gold[0].gold >= 20 && newIndexes.length != 0){
        hint_box.style.display = "none"
        gold[0].gold -= 20;
        loadGoldBalance(0)
        ...
        tiles[newIndexes[0] + currentRow].textContent = word[newIndexes[0]]
        tiles[newIndexes[0] + currentRow].classList.add("turnGreen")
        tiles[newIndexes[0] + currentRow].style.backgroundColor = "green"
        newIndexes.splice(0, 1)
        
        if(newIndexes.length == 0){
            if(row == 1){
                if(tiles[0].textContent.length == 1 &&
                   tiles[1].textContent.length == 1 &&
                   tiles[2].textContent.length == 1 &&
                   tiles[3].textContent.length == 1 &&
                   tiles[4].textContent.length == 1
                ){
                    row1 = 4
                    SubmitButton()
                } 
            } else if(row == 2){
                if(tiles[5].textContent.length == 1 &&
                    tiles[6].textContent.length == 1 &&
                    tiles[7].textContent.length == 1 &&
                    tiles[8].textContent.length == 1 &&
                    tiles[9].textContent.length == 1
                 ){
                     row2 = 9
                     SubmitButton()
                 }
            } else if(row == 3){
                if(tiles[10].textContent.length == 1 &&
                    tiles[11].textContent.length == 1 &&
                    tiles[12].textContent.length == 1 &&
                    tiles[13].textContent.length == 1 &&
                    tiles[14].textContent.length == 1
                 ){
                     row3 = 14
                     SubmitButton()
                 }
            } else if(row == 4){
                if(tiles[15].textContent.length == 1 &&
                    tiles[16].textContent.length == 1 &&
                    tiles[17].textContent.length == 1 &&
                    tiles[18].textContent.length == 1 &&
                    tiles[19].textContent.length == 1
                 ){
                     row4 = 19
                     SubmitButton()
                 }
            } else if(row == 5){
                if(tiles[20].textContent.length == 1 &&
                    tiles[21].textContent.length == 1 &&
                    tiles[22].textContent.length == 1 &&
                    tiles[23].textContent.length == 1 &&
                    tiles[24].textContent.length == 1
                 ){
                     row5 = 24
                     SubmitButton()
                 }
            } else if(row == 6){
                if(tiles[25].textContent.length == 1 &&
                    tiles[26].textContent.length == 1 &&
                    tiles[27].textContent.length == 1 &&
                    tiles[28].textContent.length == 1 &&
                    tiles[29].textContent.length == 1
                 ){
                     row6 = 29
                     SubmitButton()
                 }
...
function GameStart(){ 
    score = 0
    row1 = -1, row2 = 4, row3 = 9, row4 = 14, row5 = 19, row6 = 24
    row = 1;
    col = -1
    keys = true, GameRunning = true;
    word = wordList[Math.floor(Math.random() * wordList.length - 1 ) + 1].toLocaleUpperCase()
    console.log(word)
    tileComplete = false
    guess = ""
    c = -1, r = -1, e = 0, i = 0
    maximum, permaximum = 5, renewC, renewR
    consIndex = 5
...
function SubmitButton(){
    ...
    function checkAnswerPerTile(par){
        if(par == 1){
            maximum = 5
            renewC = 0;
            renewR = 0
                ...
                if(word == guess){
                    while(r != maximum){
                        if(word[i] == tiles[r].textContent){
                            ...
                            tiles[r].style.backgroundColor = "green"
                        }
                        i++
                        r++
                        if(r == maximum -1){
                    ...
                    while(r != maximum){
                        while(c != maximum){
                            if(tiles[r].textContent == word[i]){            
                                tiles[r].classList.replace("AnInput","turnGreen")
                                tiles[r].style.backgroundColor = "green"
                                for(let x = 0; x < 5; x++){
                                    console.log(r)
                                    if(newIndexes[x] == r - currentRow){
                                        newIndexes.splice(x, 1)
                                        break
                                    }
                                }
                                break
                            } else if(tiles[r].textContent != word[e]){
                                tiles[r].classList.replace("AnInput", "turnGrey")
                                tiles[r].style.backgroundColor = "gray"
                            } else if(tiles[r].textContent == word[e]){
                                tiles[r].classList.replace("AnInput","turnYellow")
                                tiles[r].style.backgroundColor = "yellow"
                                break
                            }
                            e++
                            c++
...
function KeyBoardColor(){
    ...
    while(r != maximum){
        for(let l = 0; l < 28; l++){
            ...
            } else if(tiles[r].style.backgroundColor == "yellow"){
                if(tiles[r].textContent.toLowerCase() == keyboardKeys[l].textContent){
                    if(keyboardKeys[l].style.backgroundColor != "green"){
                        keyboardKeys[l].style.backgroundColor = "yellow"
                    }
                }
            } else if(tiles[r].style.backgroundColor == "gray"){
                if(tiles[r].textContent.toLowerCase() == keyboardKeys[l].textContent){
                    keyboardKeys[l].style.backgroundColor = "gray"
                }
            }
        }
        r++
...
function NextWord(){
    ...
    for(let q = 0; q < 30 ; q++){
        ...
        tiles[q].textContent = ""
        tiles[q].style.backgroundColor = "#8a8a72d2"
